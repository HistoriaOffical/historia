...
// which are part of the Historia repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="historia-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /historia-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "historia-historia/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from historia-historia/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'historia-historia/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from historia-historia/develop"
            }
          }

          if (hasCache) {
            sh "cd /historia-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /historia-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /historia-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /historia-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /historia-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /historia-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /historia-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
