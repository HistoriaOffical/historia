...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "outputs") {
        ...
        pwalletMain->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_100);
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Historia address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
    ...
    if (strCommand == "create-alias")
    {
        ...
        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            if(mne.getAlias() == strAlias) {
                ...
                bool fResult = CMasternodeBroadcast::Create(mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), strError, mnb, mne.getIpv6(), mne.getIpfsId(), true);
