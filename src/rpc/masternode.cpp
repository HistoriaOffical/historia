...
void masternode_list_help()
{
    throw std::runtime_error(
            ...
            "  owneraddress   - Print the masternode owner Historia address\n"
            "  payee          - Print the masternode payout Historia address (can be additionally filtered,\n"
            "                   partial match)\n"
            "  pubKeyOperator - Print the masternode operator public key\n"
            "  status         - Print masternode status: ENABLED / POSE_BANNED\n"
            "                   (can be additionally filtered, partial match)\n"
            "  votingaddress  - Print the masternode voting Historia address\n"
        );
}
...
UniValue masternode_outputs(const JSONRPCRequest& request)
{
    ...
    pwallet->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_100);
    ...
    pwallet->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_5000);
    for (const auto& out : vPossibleCoins) {
        obj.push_back(Pair(out.tx->GetHash().ToString(), strprintf("%d", out.i)));
    }
