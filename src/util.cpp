...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty historia.conf if it does not excist
        FILE* configFile = fopen(GetConfigFile(confPath).string().c_str(), "a");
        if (configFile != NULL) {
            //Add parameters to config if conf doesn't exist for HLWA intergration
            fprintf(configFile, "#Autogenerated settings for HLWA\n");
            fprintf(configFile, "#If HLWA isn't installed you can change these things\n");
            fprintf(configFile, "#If HLWA is installed, and you will have to reimport these settings if you change them\n");
