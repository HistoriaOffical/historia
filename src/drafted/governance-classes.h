 ...
 *
 *   // network
 *   CHistoriaNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.historia.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CHistoriaNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CHistoriaNetwork(UniValue objIn)
    {
...
// // can be under: HistoriaNetwork
// //   -- signature requirements : Key1(User)
// class CHistoriaNetworkVariable : public CGovernanceObject
// {
// private:
