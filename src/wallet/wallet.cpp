        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = pcoin->tx->vout[i].nValue == 100*COIN;
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasternodeMode && wtx.tx->vout[i].nValue == 100*COIN) continue;
...
bool CWallet::SelectPrivateCoins(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 100*COIN) continue; //masternode input
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 5000*COIN) continue; //masternode input
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, const std::string& strTxHash, const std::string& strOutputIndex)
{
    ...
    if (vPossibleCoins.empty() && vPossibleCoins5000.empty()) {
    ...
    for (const auto& out : vPossibleCoins5000)
        if(out.tx->GetHash() == txHash && out.i == nOutputIndex) // found it!
            return GetOutpointAndKeysFromOutput(out, outpointRet, pubKeyRet, keyRet);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 HTA."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-historia-address
                        CScript scriptChange;
