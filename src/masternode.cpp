  ...
  //          LogPrint("masternode", "CMasternode::Check -- Masternode UTXO collateral is 5000, masternode=%s\n", vin.prevout.ToStringShort());
    ...
    if (sporkManager.IsSporkActive(SPORK_15_REQUIRE_IPFS_FIELD) && fMasterNode) {
        ...
        try {
            ...
            if (type == 2) {
                bool fIPFSActive = masternodeSync.IsSynced() && mnodeman.IsIPFSActive(type, fOurMasternode, GetIpfsId(), GetIpv6(), Ipv4) && mnodeman.IsIPFSGatewayActive(type, fOurMasternode, GetIpfsId(), GetIpv6(), Ipv4, true);
                if (!fIPFSActive) {
            ...
            if (type == 2) {
                bool fIPFSActive = masternodeSync.IsSynced() && mnodeman.IsIPFSGatewayActive(type, fOurMasternode, GetIpfsId(), GetIpv6(), Ipv4, false);
                if (!fIPFSActive) {
                    nActiveState = MASTERNODE_IPFS_EXPIRED;
                    if (nActiveStatePrev != nActiveState) {
                        LogPrint("masternode", "CMasternode::Check -- Masternode %s is in %s state now\n", vin.prevout.ToStringShort(), GetStateString());
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    // should be at least not earlier than block when 100 HTA tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 100 HTA tx -> 1 confirmation
